...
class DLL_EXPORT COutputQueue : public CCritSec
{
public:
    //  Constructor
    DLL_EXPORT
    ...
    DLL_EXPORT ~COutputQueue();

    // enter flush state - discard all data
    void DLL_EXPORT BeginFlush();      // Begin flushing samples

    // re-enable receives (pass this downstream)
    void DLL_EXPORT EndFlush();        // Complete flush of samples - downstream
                            // pin guaranteed not to block at this stage

    void DLL_EXPORT EOS();             // Call this on End of stream

    void DLL_EXPORT SendAnyway();      // Send batched samples anyway (if bBatchExact set)

    void DLL_EXPORT NewSegment(
            REFERENCE_TIME tStart,
            REFERENCE_TIME tStop,
            double dRate);

    HRESULT DLL_EXPORT Receive(IMediaSample *pSample);

    // do something with these media samples
    HRESULT DLL_EXPORT ReceiveMultiple (
        __in_ecount(nSamples) IMediaSample **pSamples,
        long nSamples,
        __out long *nSamplesProcessed);

    void DLL_EXPORT Reset();           // Reset m_hr ready for more data

    //  See if its idle or not
    BOOL DLL_EXPORT IsIdle();

    // give the class an event to fire after everything removed from the queue
    void DLL_EXPORT SetPopEvent(HANDLE hEvent);

protected:
    static DWORD WINAPI DLL_EXPORT InitialThreadProc(__in LPVOID pv);
    DWORD DLL_EXPORT ThreadProc();
    BOOL  DLL_EXPORT IsQueued()
    {
        return m_List != NULL;
    };

    //  The critical section MUST be held when this is called
    void DLL_EXPORT QueueSample(IMediaSample *pSample);

    BOOL DLL_EXPORT IsSpecialSample(IMediaSample *pSample)
    {
        return (DWORD_PTR)pSample > (DWORD_PTR)(LONG_PTR)(-16);
    };

    //  Remove and Release() batched and queued samples
    void DLL_EXPORT FreeSamples();

    //  Notify the thread there is something to do
    void DLL_EXPORT NotifyThread();
...
protected:
    ...
    struct DLL_EXPORT NewSegmentPacket {
        REFERENCE_TIME tStart;
        REFERENCE_TIME tStop;
        double dRate;
    };
