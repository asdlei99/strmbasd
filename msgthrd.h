...
public:
    ...
    DLL_EXPORT CMsg(UINT u, DWORD dw, __inout_opt LPVOID lp, __in_opt CAMEvent *pEvnt)
...
//
class AM_NOVTABLE DLL_EXPORT CMsgThread {
private:
    static DWORD WINAPI DLL_EXPORT DefaultThreadProc(__inout LPVOID lpParam);
    DWORD               m_ThreadId;
    HANDLE              m_hThread;
...
public:
    ...
    // as the message loop
    void virtual DLL_EXPORT GetThreadMsg(__out CMsg *msg);

    // override this if you want to do something on thread startup
    virtual void DLL_EXPORT OnThreadInit() {
    };

    BOOL DLL_EXPORT CreateThread();

    BOOL DLL_EXPORT WaitForThreadExit(__out LPDWORD lpdwExitCode) {
        if (m_hThread != NULL) {
    ...
    DWORD DLL_EXPORT ResumeThread() {
        return ::ResumeThread(m_hThread);
    }

    DWORD DLL_EXPORT SuspendThread() {
        return ::SuspendThread(m_hThread);
    }

    int DLL_EXPORT GetThreadPriority() {
        return ::GetThreadPriority(m_hThread);
    }

    BOOL DLL_EXPORT SetThreadPriority(int nPriority) {
        return ::SetThreadPriority(m_hThread, nPriority);
    }

    HANDLE DLL_EXPORT GetThreadHandle() {
        return m_hThread;
    }

    DWORD DLL_EXPORT GetThreadId() {
        return m_ThreadId;
    }


    void DLL_EXPORT PutThreadMsg(UINT uMsg, DWORD dwMsgFlags,
    ...
    // the creator thread.
    //
    virtual LRESULT DLL_EXPORT ThreadMessageProc(
