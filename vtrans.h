...
class DLL_EXPORT CVideoTransformFilter : public CTransformFilter
{
  public:

    DLL_EXPORT CVideoTransformFilter(__in_opt LPCTSTR, __inout_opt LPUNKNOWN, REFCLSID clsid);
    DLL_EXPORT ~CVideoTransformFilter();
    HRESULT DLL_EXPORT EndFlush();
  ...
  protected:
    ...
    HRESULT DLL_EXPORT AbortPlayback(HRESULT hr);	// if something bad happens

    HRESULT DLL_EXPORT Receive(IMediaSample *pSample);

    HRESULT DLL_EXPORT AlterQuality(Quality q);

    BOOL DLL_EXPORT ShouldSkipFrame(IMediaSample * pIn);
