...
class DLL_EXPORT CCritSec {

    // make copy constructor and assignment operator inaccessible

    DLL_EXPORT CCritSec(const CCritSec &refCritSec);
    DLL_EXPORT CCritSec &operator=(const CCritSec &refCritSec);
...
public:
    DLL_EXPORT CCritSec();
    DLL_EXPORT ~CCritSec();
    void DLL_EXPORT Lock();
    void DLL_EXPORT Unlock();
#else
...
#ifdef DEBUG
    BOOL WINAPI DLL_EXPORT CritCheckIn(CCritSec * pcCrit);
    BOOL WINAPI DLL_EXPORT CritCheckIn(const CCritSec * pcCrit);
    BOOL WINAPI DLL_EXPORT CritCheckOut(CCritSec * pcCrit);
    BOOL WINAPI DLL_EXPORT CritCheckOut(const CCritSec * pcCrit);
    void WINAPI DLL_EXPORT DbgLockTrace(CCritSec * pcCrit, BOOL fTrace);
#else
...
class DLL_EXPORT CAutoLock {

    // make copy constructor and assignment operator inaccessible

    DLL_EXPORT CAutoLock(const CAutoLock &refAutoLock);
    CAutoLock DLL_EXPORT &operator=(const CAutoLock &refAutoLock);

protected:
    CCritSec * m_pLock;

public:
    DLL_EXPORT CAutoLock(CCritSec * plock)
    {
        m_pLock = plock;
        m_pLock->Lock();
    };

    DLL_EXPORT ~CAutoLock() {
        m_pLock->Unlock();
    };
};
...
class DLL_EXPORT CAMEvent
{

    // make copy constructor and assignment operator inaccessible

    DLL_EXPORT CAMEvent(const CAMEvent &refEvent);
    CAMEvent DLL_EXPORT &operator=(const CAMEvent &refEvent);

protected:
    HANDLE m_hEvent;
public:
    DLL_EXPORT CAMEvent(BOOL fManualReset = FALSE, __inout_opt HRESULT *phr = NULL);
    DLL_EXPORT CAMEvent(__inout_opt HRESULT *phr);
    DLL_EXPORT ~CAMEvent();
    ...
    void DLL_EXPORT Reset() { ResetEvent(m_hEvent); };
    BOOL DLL_EXPORT Check() { return Wait(0); };
};
...
class DLL_EXPORT CAMMsgEvent : public CAMEvent
{

public:

    DLL_EXPORT CAMMsgEvent(__inout_opt HRESULT *phr = NULL);

    // Allow SEND messages to be processed while waiting
    BOOL DLL_EXPORT WaitMsg(DWORD dwTimeout = INFINITE);
};
...
class AM_NOVTABLE DLL_EXPORT CAMThread {

    // make copy constructor and assignment operator inaccessible

    DLL_EXPORT CAMThread(const CAMThread &refThread);
    DLL_EXPORT CAMThread &operator=(const CAMThread &refThread);
...
public:
    DLL_EXPORT CAMThread(__inout_opt HRESULT *phr = NULL);
    virtual DLL_EXPORT ~CAMThread();
    ...
    static DWORD WINAPI DLL_EXPORT InitialThreadProc(__inout LPVOID pv);

    // start thread running  - error if already running
    BOOL DLL_EXPORT Create();

    // signal the thread, and block for a response
    //
    DWORD DLL_EXPORT CallWorker(DWORD);
...
//#pragma warning(pop)
    ...
    DWORD DLL_EXPORT GetRequest();

    // is there a request?
    BOOL DLL_EXPORT CheckRequest(__out_opt DWORD * pParam);

    // reply to the request
    void DLL_EXPORT Reply(DWORD);
    ...
    static HRESULT DLL_EXPORT CoInitializeHelper();
};
#endif // AM_NOVTABLE
...
extern const DWORD DLL_EXPORT bits555[3];
extern const DWORD DLL_EXPORT bits565[3];
extern const DWORD DLL_EXPORT bits888[3];
...
STDAPI DLL_EXPORT AMGetWideString(LPCWSTR pszString, __deref_out LPWSTR *ppszReturn);
