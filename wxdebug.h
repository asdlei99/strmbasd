...
#define DLLEXPORT __declspec(dllexport)
    ...
    void WINAPI DLL_EXPORT DbgInitKeyLevels(HKEY hKey, bool fTakeMax);
    void WINAPI DLL_EXPORT DbgInitGlobalSettings(bool fTakeMax);
    void WINAPI DLL_EXPORT DbgInitModuleSettings(bool fTakeMax);
    void WINAPI DLL_EXPORT DbgInitModuleName();
    DWORD WINAPI DLL_EXPORT DbgRegisterObjectCreation(
        LPCSTR szObjectName, LPCWSTR wszObjectName);

    BOOL WINAPI DLL_EXPORT DbgRegisterObjectDestruction(DWORD dwCookie);

    // These are the PUBLIC entry points

    BOOL WINAPI DLL_EXPORT DbgCheckModuleLevel(DWORD Type,DWORD Level);
    void WINAPI DLL_EXPORT DbgSetModuleLevel(DWORD Type,DWORD Level);
    void WINAPI DLL_EXPORT DbgSetAutoRefreshLevels(bool fAuto);

    // Initialise the library with the module handle

    void WINAPI DLL_EXPORT DbgInitialise(HINSTANCE hInst);
    void WINAPI DLL_EXPORT DbgTerminate();

    void WINAPI DLL_EXPORT DbgDumpObjectRegister();

    // Display error and logging to the user

    void WINAPI DLL_EXPORT DbgAssert(LPCTSTR pCondition,LPCTSTR pFileName,INT iLine);
    void WINAPI DLL_EXPORT DbgBreakPoint(LPCTSTR pCondition,LPCTSTR pFileName,INT iLine);
    void WINAPI DLL_EXPORT DbgBreakPoint(LPCTSTR pFileName,INT iLine,__format_string LPCTSTR  szFormatString,...);

    void WINAPI DLL_EXPORT DbgKernelAssert(LPCTSTR pCondition,LPCTSTR pFileName,INT iLine);
    void WINAPI DLL_EXPORT DbgLogInfo(DWORD Type,DWORD Level,__format_string LPCTSTR pFormat,...);
#ifdef UNICODE
    void WINAPI DLL_EXPORT DbgLogInfo(DWORD Type,DWORD Level,__format_string LPCSTR pFormat,...);
    void WINAPI DLL_EXPORT DbgAssert(LPCSTR pCondition,LPCSTR pFileName,INT iLine);
    void WINAPI DLL_EXPORT DbgBreakPoint(LPCSTR pCondition,LPCSTR pFileName,INT iLine);
    void WINAPI DLL_EXPORT DbgKernelAssert(LPCSTR pCondition,LPCSTR pFileName,INT iLine);
#endif
    void WINAPI DLL_EXPORT DbgOutString(LPCTSTR psz);

    //  Debug infinite wait stuff
    DWORD WINAPI DLL_EXPORT DbgWaitForSingleObject(HANDLE h);
    DWORD WINAPI DLL_EXPORT DbgWaitForMultipleObjects(DWORD nCount,
                                    __in_ecount(nCount) CONST HANDLE *lpHandles,
                                    BOOL bWaitAll);
    void WINAPI DLL_EXPORT DbgSetWaitTimeout(DWORD dwTimeout);

#ifdef __strmif_h__
    // Display a media type: Terse at level 2, verbose at level 5
    void WINAPI DLL_EXPORT DisplayType(LPCTSTR label, const AM_MEDIA_TYPE *pmtIn);

    // Dump lots of information about a filter graph
    void WINAPI DLL_EXPORT DumpGraph(IFilterGraph *pGraph, DWORD dwLevel);
#endif
...
class DLL_EXPORT CDispBasic
{
public:
    DLL_EXPORT CDispBasic() { m_pString = m_String; };
    DLL_EXPORT ~CDispBasic();
protected:
    PTCHAR m_pString;  // normally points to m_String... unless too much data
    TCHAR m_String[50];
};
class DLL_EXPORT CDisp : public CDispBasic
{
public:
    DLL_EXPORT CDisp(LONGLONG ll, int Format = CDISP_HEX); // Display a LONGLONG in CDISP_HEX or CDISP_DEC form
    DLL_EXPORT CDisp(REFCLSID clsid);      // Display a GUID
    DLL_EXPORT CDisp(double d);            // Display a floating point number
#ifdef __strmif_h__
#ifdef __STREAMS__
    DLL_EXPORT CDisp(CRefTime t);          // Display a Reference Time
#endif
    DLL_EXPORT CDisp(IPin *pPin);          // Display a pin as {filter clsid}(pin name)
    DLL_EXPORT CDisp(IUnknown *pUnk);      // Display a filter or pin
#endif // __strmif_h__
    DLL_EXPORT ~CDisp();
