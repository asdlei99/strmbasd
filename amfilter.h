...
class DLL_EXPORT CBaseMediaFilter;     // IMediaFilter support
class DLL_EXPORT CBaseFilter;          // IBaseFilter,IMediaFilter support
class DLL_EXPORT CBasePin;             // Abstract base class for IPin interface
class DLL_EXPORT CEnumPins;            // Enumerate input and output pins
class DLL_EXPORT CEnumMediaTypes;      // Enumerate the pin's preferred formats
class DLL_EXPORT CBaseOutputPin;       // Adds data provider member functions
class DLL_EXPORT CBaseInputPin;        // Implements IMemInputPin interface
class DLL_EXPORT CMediaSample;         // Basic transport unit for IMemInputPin
class DLL_EXPORT CBaseAllocator;       // General list guff for most allocators
class DLL_EXPORT CMemAllocator;        // Implements memory buffer allocation
...
public:
    ...
    virtual DLL_EXPORT ~CBaseMediaFilter();
    ...
    STDMETHODIMP DLL_EXPORT GetState(DWORD dwMSecs, __out FILTER_STATE *State);

    STDMETHODIMP DLL_EXPORT SetSyncSource(__inout_opt IReferenceClock *pClock);

    STDMETHODIMP DLL_EXPORT GetSyncSource(__deref_out_opt IReferenceClock **pClock);
    ...
    // its presentation
    STDMETHODIMP DLL_EXPORT Run(REFERENCE_TIME tStart);

    // --- helper methods ---

    // return the current stream time - ie find out what
    // stream time should be appearing now
    virtual HRESULT DLL_EXPORT StreamTime(CRefTime& rtStream);

    // Is the filter currently active? (running or paused)
    BOOL DLL_EXPORT IsActive() {
...
public:
    ...
    STDMETHODIMP DLL_EXPORT SetSyncSource(__in_opt IReferenceClock *pClock);
    ...
    // pin enumerator
    STDMETHODIMP DLL_EXPORT EnumPins(
    ...
    STDMETHODIMP DLL_EXPORT QueryFilterInfo(
                    __out FILTER_INFO * pInfo);

    STDMETHODIMP DLL_EXPORT JoinFilterGraph(
    ...
    STDMETHODIMP DLL_EXPORT QueryVendorInfo(
    ...
    HRESULT DLL_EXPORT NotifyEvent(
        long EventCode,
        LONG_PTR EventParam1,
        LONG_PTR EventParam2);

    // return the filter graph we belong to
    __out_opt IFilterGraph * DLL_EXPORT GetFilterGraph() {
        return m_pGraph;
    }
    ...
    HRESULT DLL_EXPORT ReconnectPin(IPin *pPin, __in_opt AM_MEDIA_TYPE const *pmt);

    // find out the current pin version (used by enumerators)
    virtual LONG DLL_EXPORT GetPinVersion();
    void DLL_EXPORT IncrementPinVersion();

    // you need to supply these to access the pins from the enumerator
    // and for default Stop and Pause/Run activation.
    virtual int DLL_EXPORT GetPinCount() PURE;
    virtual CBasePin* DLL_EXPORT GetPin(int n) PURE;

    // --- IAMovieSetup methods ---

    STDMETHODIMP DLL_EXPORT Register();    // ask filter to register itself
    STDMETHODIMP DLL_EXPORT Unregister();  // and unregister itself

    // --- setup helper methods ---
    // (override to return filters setup data)

    virtual __out_opt LPAMOVIESETUP_FILTER DLL_EXPORT GetSetupData(){ return NULL; }
...
protected:
...
#ifdef DEBUG
    void DLL_EXPORT DisplayPinInfo(IPin *pReceivePin);
    void DLL_EXPORT DisplayTypeInfo(IPin *pPin, const CMediaType *pmt);
#else
    void DLL_EXPORT DisplayPinInfo(IPin *pReceivePin) {};
    void DLL_EXPORT DisplayTypeInfo(IPin *pPin, const CMediaType *pmt) {};
#endif
    ...
    HRESULT DLL_EXPORT TryMediaTypes(
                        IPin *pReceivePin,          // connect to this pin
    ...
    HRESULT DLL_EXPORT AgreeMediaType(
                        IPin *pReceivePin,      // connect to this pin
...
public:
    ...
    STDMETHODIMP DLL_EXPORT Connect(
        IPin * pReceivePin,
        __in_opt const AM_MEDIA_TYPE *pmt   // optional media type
    );

    // (passive) accept a connection from another pin
    STDMETHODIMP DLL_EXPORT ReceiveConnection(
        IPin * pConnector,      // this is the initiating connecting pin
        const AM_MEDIA_TYPE *pmt   // this is the media type we will exchange
    );

    STDMETHODIMP DLL_EXPORT Disconnect();

    STDMETHODIMP DLL_EXPORT ConnectedTo(__deref_out IPin **pPin);

    STDMETHODIMP DLL_EXPORT ConnectionMediaType(__out AM_MEDIA_TYPE *pmt);

    STDMETHODIMP DLL_EXPORT QueryPinInfo(
        __out PIN_INFO * pInfo
    );

    STDMETHODIMP DLL_EXPORT QueryDirection(
    ...
    STDMETHODIMP DLL_EXPORT QueryAccept(
        const AM_MEDIA_TYPE *pmt
    );

    // return an enumerator for this pins preferred media types
    STDMETHODIMP DLL_EXPORT EnumMediaTypes(
    ...
    STDMETHODIMP DLL_EXPORT QueryInternalConnections(
    ...
    STDMETHODIMP DLL_EXPORT SetSink(IQualityControl * piqc);

    // --- helper methods ---

    // Returns true if the pin is connected. false otherwise.
    BOOL DLL_EXPORT IsConnected(void) {return (m_Connected != NULL); };
    // Return the pin this is connected to (if any)
    IPin* DLL_EXPORT GetConnected() { return m_Connected; };
    ...
    virtual LONG DLL_EXPORT GetMediaTypeVersion();
    void DLL_EXPORT IncrementTypeVersion();

    // switch the pin to active (paused or running) mode
    // not an error to call this if already active
    virtual HRESULT DLL_EXPORT Active(void);

    // switch the pin to inactive state - may already be inactive
    virtual HRESULT DLL_EXPORT Inactive(void);

    // Notify of Run() from filter
    virtual HRESULT DLL_EXPORT Run(REFERENCE_TIME tStart);

    // check if the pin can support this specific proposed type and format
    virtual HRESULT DLL_EXPORT CheckMediaType(const CMediaType *) PURE;

    // set the connection to use this format (previously agreed)
    virtual HRESULT DLL_EXPORT SetMediaType(const CMediaType *);

    // check that the connection is ok before verifying it
    // can be overridden eg to check what interfaces will be supported.
    virtual HRESULT DLL_EXPORT CheckConnect(IPin *);

    // Set and release resources required for a connection
    virtual HRESULT DLL_EXPORT BreakConnect();
    virtual HRESULT DLL_EXPORT CompleteConnect(IPin *pReceivePin);

    // returns the preferred formats for a pin
    virtual HRESULT DLL_EXPORT GetMediaType(int iPosition, __inout CMediaType *pMediaType);

    // access to NewSegment values
    REFERENCE_TIME DLL_EXPORT CurrentStopTime() {
        return m_tStop;
    }
    REFERENCE_TIME DLL_EXPORT CurrentStartTime() {
        return m_tStart;
    }
    double DLL_EXPORT CurrentRate() {
        return m_dRate;
    }

    //  Access name
    LPWSTR DLL_EXPORT Name() { return m_pName; };

    //  Can reconnectwhen active?
    void DLL_EXPORT SetReconnectWhenActive(bool bCanReconnect)
    {
        m_bCanReconnectWhenActive = bCanReconnect;
    }

    bool DLL_EXPORT CanReconnectWhenActive()
    {
        return m_bCanReconnectWhenActive;
    }

protected:
    STDMETHODIMP DLL_EXPORT DisconnectInternal();
};
...
class CEnumPins : public IEnumPins      // The interface we support
{
    ...
    BOOL DLL_EXPORT AreWeOutOfSync() {
    ...
    STDMETHODIMP DLL_EXPORT Refresh();
...
public:
    ...
    virtual DLL_EXPORT ~CEnumPins();

    // IUnknown
    STDMETHODIMP DLL_EXPORT QueryInterface(REFIID riid, __deref_out void **ppv);
    STDMETHODIMP_(ULONG) DLL_EXPORT AddRef();
    STDMETHODIMP_(ULONG) DLL_EXPORT Release();
    ...
    STDMETHODIMP DLL_EXPORT Skip(ULONG cPins);
    STDMETHODIMP DLL_EXPORT Reset();
    STDMETHODIMP DLL_EXPORT Clone(__deref_out IEnumPins **ppEnum);
...
public:
    ...
    virtual DLL_EXPORT ~CEnumMediaTypes();
    ...
    STDMETHODIMP DLL_EXPORT Skip(ULONG cMediaTypes);
    STDMETHODIMP DLL_EXPORT Reset();
    STDMETHODIMP DLL_EXPORT Clone(__deref_out IEnumMediaTypes **ppEnum);
};
...
public:
    ...
    virtual HRESULT DLL_EXPORT DecideAllocator(IMemInputPin * pPin, __deref_out IMemAllocator ** pAlloc);
    ...
    virtual HRESULT DLL_EXPORT GetDeliveryBuffer(__deref_out IMediaSample ** ppSample,
    ...
    // call.
    virtual HRESULT DLL_EXPORT Deliver(IMediaSample *);

    // override this to control the connection
    virtual HRESULT DLL_EXPORT InitAllocator(__deref_out IMemAllocator **ppAlloc);
    ...
    virtual HRESULT DLL_EXPORT DeliverEndOfStream(void);
    ...
    virtual HRESULT DLL_EXPORT DeliverBeginFlush(void);
    virtual HRESULT DLL_EXPORT DeliverEndFlush(void);

    // deliver NewSegment to connected pin - you will need to
    // override this if you queue any data in your output pin.
    virtual HRESULT DLL_EXPORT DeliverNewSegment(
                        REFERENCE_TIME tStart,
                        REFERENCE_TIME tStop,
                        double dRate);
...
public:
    ...
    CBaseInputPin(
        ...
        __in_opt LPCWSTR pName);
#endif
    virtual DLL_EXPORT ~CBaseInputPin();
    ...
    STDMETHODIMP DLL_EXPORT GetAllocator(__deref_out IMemAllocator ** ppAllocator);

    // tell the input pin which allocator the output pin is actually
    // going to use.
    STDMETHODIMP DLL_EXPORT NotifyAllocator(
    ...
    STDMETHODIMP DLL_EXPORT ReceiveMultiple (
        __in_ecount(nSamples) IMediaSample **pSamples,
        long nSamples,
        __out long *nSamplesProcessed);

    // See if Receive() blocks
    STDMETHODIMP DLL_EXPORT ReceiveCanBlock();
    ...
    // allocator
    STDMETHODIMP DLL_EXPORT GetAllocatorRequirements(__out ALLOCATOR_PROPERTIES*pProps);

    // Release the pin's allocator.
    HRESULT DLL_EXPORT BreakConnect();

    // helper method to check the read-only flag
    BOOL DLL_EXPORT IsReadOnly() {
        return m_bReadOnly;
    };

    // helper method to see if we are flushing
    BOOL DLL_EXPORT IsFlushing() {
        return m_bFlushing;
    };
    ...
    HRESULT DLL_EXPORT PassNotify(Quality& q);
    ...
    AM_SAMPLE2_PROPERTIES * DLL_EXPORT SampleProps() {
...
public:
    ...
    // IPin Methods
    STDMETHODIMP DLL_EXPORT Disconnect(void);

    // IPinFlowControl Methods
    STDMETHODIMP DLL_EXPORT Block(DWORD dwBlockFlags, HANDLE hEvent);

    //  Set graph config info
    void DLL_EXPORT SetConfigInfo(IGraphConfig *pGraphConfig, HANDLE hStopEvent);

    #ifdef DEBUG
    virtual HRESULT DLL_EXPORT Deliver(IMediaSample *pSample);
    virtual HRESULT DLL_EXPORT DeliverEndOfStream(void);
    virtual HRESULT DLL_EXPORT DeliverNewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop, double dRate);
    #endif // DEBUG

    HRESULT DLL_EXPORT DeliverBeginFlush(void);
    HRESULT DLL_EXPORT DeliverEndFlush(void);

    HRESULT DLL_EXPORT Inactive(void);
    HRESULT DLL_EXPORT Active(void);
    virtual HRESULT DLL_EXPORT CompleteConnect(IPin *pReceivePin);

    virtual HRESULT DLL_EXPORT StartUsingOutputPin(void);
    virtual void DLL_EXPORT StopUsingOutputPin(void);
    virtual bool DLL_EXPORT StreamingThreadUsingOutputPin(void);
    ...
    HRESULT DLL_EXPORT ChangeMediaType(const CMediaType *pmt);
    HRESULT DLL_EXPORT DynamicReconnect(const CMediaType *pmt);

protected:
    HRESULT DLL_EXPORT SynchronousBlockOutputPin(void);
    HRESULT DLL_EXPORT AsynchronousBlockOutputPin(HANDLE hNotifyCallerPinBlockedEvent);
    HRESULT DLL_EXPORT UnblockOutputPin(void);

    void DLL_EXPORT BlockOutputPin(void);
    void DLL_EXPORT ResetBlockState(void);

    static HRESULT DLL_EXPORT WaitEvent(HANDLE hEvent);
...
private:
    HRESULT DLL_EXPORT Initialize(void);
    HRESULT DLL_EXPORT ChangeMediaTypeHelper(const CMediaType *pmt);

    #ifdef DEBUG
    void DLL_EXPORT AssertValid(void);
    #endif // DEBUG
};
...
{
public:
    DLL_EXPORT CAutoUsingOutputPin( __in CDynamicOutputPin* pOutputPin, __inout HRESULT* phr );
    DLL_EXPORT ~CAutoUsingOutputPin();
...
public:
    ...
    virtual DLL_EXPORT ~CMediaSample();
    ...
    HRESULT DLL_EXPORT SetPointer(__in_bcount(cBytes) BYTE * ptr, LONG cBytes);

    // Get me a read/write pointer to this buffer's memory.
    STDMETHODIMP DLL_EXPORT GetPointer(__deref_out BYTE ** ppBuffer);

    STDMETHODIMP_(LONG) DLL_EXPORT GetSize(void);

    // get the stream time at which this sample should start and finish.
    STDMETHODIMP DLL_EXPORT GetTime(
        __out REFERENCE_TIME * pTimeStart,     // put time here
        __out REFERENCE_TIME * pTimeEnd
    );

    // Set the stream time at which this sample should start and finish.
    STDMETHODIMP DLL_EXPORT SetTime(
        __in_opt REFERENCE_TIME * pTimeStart,     // put time here
        __in_opt REFERENCE_TIME * pTimeEnd
    );
    STDMETHODIMP DLL_EXPORT IsSyncPoint(void);
    STDMETHODIMP DLL_EXPORT SetSyncPoint(BOOL bIsSyncPoint);
    STDMETHODIMP DLL_EXPORT IsPreroll(void);
    STDMETHODIMP DLL_EXPORT SetPreroll(BOOL bIsPreroll);

    STDMETHODIMP_(LONG) DLL_EXPORT GetActualDataLength(void);
    STDMETHODIMP DLL_EXPORT SetActualDataLength(LONG lActual);

    // these allow for limited format changes in band

    STDMETHODIMP DLL_EXPORT GetMediaType(__deref_out AM_MEDIA_TYPE **ppMediaType);
    STDMETHODIMP DLL_EXPORT SetMediaType(__in_opt AM_MEDIA_TYPE *pMediaType);

    // returns S_OK if there is a discontinuity in the data (this same is
    // not a continuation of the previous stream of data
    // - there has been a seek).
    STDMETHODIMP DLL_EXPORT IsDiscontinuity(void);
    // set the discontinuity property - TRUE if this sample is not a
    // continuation, but a new sample after a seek.
    STDMETHODIMP DLL_EXPORT SetDiscontinuity(BOOL bDiscontinuity);

    // get the media times for this sample
    STDMETHODIMP DLL_EXPORT GetMediaTime(
    	__out LONGLONG * pTimeStart,
	    __out LONGLONG * pTimeEnd
    );

    // Set the media times for this sample
    STDMETHODIMP DLL_EXPORT SetMediaTime(
    	__in_opt LONGLONG * pTimeStart,
	    __in_opt LONGLONG * pTimeEnd
    );

    // Set and get properties (IMediaSample2)
    STDMETHODIMP DLL_EXPORT GetProperties(
        DWORD cbProperties,
                       ...
                       public CCritSec             // Provides object locking
{
    ...
    static DLL_EXPORT CMediaSample * &NextSample(__in CMediaSample *pSample)
    {
        return pSample->m_pNext;
    };

    /*  Mini list class for the free list */
    class DLL_EXPORT CSampleList
    {
    public:
        DLL_EXPORT CSampleList() : m_List(NULL), m_nOnList(0) {};
#ifdef DEBUG
        DLL_EXPORT ~CSampleList()
        {
            ASSERT(m_nOnList == 0);
        };
#endif
        CMediaSample* DLL_EXPORT Head() const { return m_List; };
        CMediaSample* DLL_EXPORT Next(__in CMediaSample *pSample) const { return CBaseAllocator::NextSample(pSample); };
        int DLL_EXPORT GetCount() const { return m_nOnList; };
        void DLL_EXPORT Add(__inout CMediaSample *pSample)
        {
            ASSERT(pSample != NULL);
        ...
        CMediaSample* DLL_EXPORT RemoveHead()
        {
        ...
        void DLL_EXPORT Remove(__inout CMediaSample *pSample);
...
protected:
    ...
    */
    ...
    virtual void DLL_EXPORT Free(void) PURE;

    // override to allocate the memory when commit called
    virtual HRESULT DLL_EXPORT Alloc(void);
...
public:
    ...
    CBaseAllocator(
        ...
        BOOL bEvent = TRUE, BOOL fEnableReleaseCallback = FALSE);
#endif
    virtual DLL_EXPORT ~CBaseAllocator();
    ...
    STDMETHODIMP DLL_EXPORT GetBuffer(__deref_out IMediaSample **ppBuffer,
                           __in_opt REFERENCE_TIME * pStartTime,
                           __in_opt REFERENCE_TIME * pEndTime,
                           DWORD dwFlags);

    // final release of a CMediaSample will call this
    STDMETHODIMP DLL_EXPORT ReleaseBuffer(IMediaSample *pBuffer);
    // obsolete:: virtual void PutOnFreeList(CMediaSample * pSample);

    STDMETHODIMP DLL_EXPORT SetNotify(IMemAllocatorNotifyCallbackTemp *pNotify);

    STDMETHODIMP DLL_EXPORT GetFreeCount(__out LONG *plBuffersFree);

    // Notify that a sample is available
    void DLL_EXPORT NotifySample();

    // Notify that we're waiting for a sample
    void DLL_EXPORT SetWaiting() { m_lWaiting++; };
};
...
STDAPI DLL_EXPORT CreateMemoryAllocator(__deref_out IMemAllocator **ppAllocator);
...
protected:
    ...
    void DLL_EXPORT Free(void);

    // called from the destructor (and from Alloc if changing size/count) to
    // actually free up the memory
    void DLL_EXPORT ReallyFree(void);

    // overriden to allocate the memory when commit called
    HRESULT DLL_EXPORT Alloc(void);

public:
    /* This goes in the factory template table to create new instances */
    static CUnknown* DLL_EXPORT CreateInstance(__inout_opt LPUNKNOWN, __inout HRESULT *);

    STDMETHODIMP DLL_EXPORT SetProperties(
		    __in ALLOCATOR_PROPERTIES* pRequest,
		    __out ALLOCATOR_PROPERTIES* pActual);

    DLL_EXPORT CMemAllocator(__in_opt LPCTSTR , __inout_opt LPUNKNOWN, __inout HRESULT *);
#ifdef UNICODE
    DLL_EXPORT CMemAllocator(__in_opt LPCSTR , __inout_opt LPUNKNOWN, __inout HRESULT *);
#endif
    DLL_EXPORT ~CMemAllocator();
};
