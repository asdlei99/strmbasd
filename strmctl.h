...
class DLL_EXPORT CBaseStreamControl : public IAMStreamControl
{
public:
...
private:
    ...
    //
    void DLL_EXPORT ExecuteStop();
    void DLL_EXPORT ExecuteStart();
    void DLL_EXPORT CancelStop();
    void DLL_EXPORT CancelStart();
    ...
    //
    enum StreamControlState DLL_EXPORT CheckSampleTimes( __in const REFERENCE_TIME * pSampleStart,
...
public:
    ...
    // below.
    //
    DLL_EXPORT CBaseStreamControl(__inout_opt HRESULT *phr = NULL);
    DLL_EXPORT ~CBaseStreamControl();
    ...
    //
    void DLL_EXPORT SetSyncSource( IReferenceClock * pRefClock )
    {
    ...
    // IMediaEventSink
    //
    void DLL_EXPORT SetFilterGraph( IMediaEventSink *pSink ) {
        m_pSink = pSink;
    }
    ...
    //
    void DLL_EXPORT NotifyFilterState( FILTER_STATE new_state, REFERENCE_TIME tStart = 0 );

    // Filter should call Flushing(TRUE) in BeginFlush,
    // and Flushing(FALSE) in EndFlush.
    //
    void DLL_EXPORT Flushing( BOOL bInProgress );
    ...
    STDMETHODIMP DLL_EXPORT StopAt( const REFERENCE_TIME * ptStop = NULL,
			 BOOL bSendExtra = FALSE,
			 DWORD dwCookie = 0 );
    STDMETHODIMP DLL_EXPORT StartAt( const REFERENCE_TIME * ptStart = NULL,
		    	  DWORD dwCookie = 0 );
    STDMETHODIMP DLL_EXPORT GetInfo( __out AM_STREAM_INFO *pInfo);
    ...
    // settings
    enum StreamControlState DLL_EXPORT CheckStreamState( IMediaSample * pSample );
...
private:
    ...
    HANDLE DLL_EXPORT GetStreamEventHandle() const { return m_StreamEvent; }
    enum StreamControlState GetStreamState() const { return m_StreamState; }
    BOOL DLL_EXPORT IsStreaming() const { return m_StreamState == STREAM_FLOWING; }
};
