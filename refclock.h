...
class DLL_EXPORT CBaseReferenceClock
: public CUnknown, public IReferenceClock, public CCritSec, public IReferenceClockTimerControl
{
protected:
    virtual DLL_EXPORT ~CBaseReferenceClock();     // Don't let me be created on the stack!
public:
    DLL_EXPORT CBaseReferenceClock(__in_opt LPCTSTR pName,
    ...
    STDMETHODIMP DLL_EXPORT GetTime(__out REFERENCE_TIME *pTime);
    // When this is called, it sets m_rtLastGotTime to the time it returns.

    /* Provide standard mechanisms for scheduling events */

    /* Ask for an async notification that a time has elapsed */
    STDMETHODIMP DLL_EXPORT AdviseTime(
    ...
    STDMETHODIMP DLL_EXPORT AdvisePeriodic(
    ...
    STDMETHODIMP DLL_EXPORT Unadvise(DWORD_PTR dwAdviseCookie);
    ...
    virtual REFERENCE_TIME DLL_EXPORT GetPrivateTime();

    /* Provide a method for correcting drift */
    STDMETHODIMP DLL_EXPORT SetTimeDelta( const REFERENCE_TIME& TimeDelta );

    CAMSchedule * DLL_EXPORT GetSchedule() const { return m_pSchedule; }

    // IReferenceClockTimerControl methods
    //
    // Setting a default of 0 disables the default of 1ms
    STDMETHODIMP DLL_EXPORT SetDefaultTimerResolution(
        REFERENCE_TIME timerResolution // in 100ns
    );
    STDMETHODIMP DLL_EXPORT GetDefaultTimerResolution(
...
// Thread stuff
public:
    void DLL_EXPORT TriggerThread()    // Wakes thread up.  Need to do this if
...
private:
    ...
    HRESULT DLL_EXPORT AdviseThread();             // Method in which the advise thread runs
    static DWORD __stdcall DLL_EXPORT AdviseThreadFunction(__in LPVOID); // Function used to get there

protected:
    CAMSchedule * m_pSchedule;

    void DLL_EXPORT Restart (IN REFERENCE_TIME rtMinTime = 0_I64) ;
};
