...
class DLL_EXPORT CPersistStream : public IPersistStream {
    private:
    ...
    public:
        ...
        STDMETHODIMP DLL_EXPORT Load(LPSTREAM pStm);
        STDMETHODIMP DLL_EXPORT Save(LPSTREAM pStm, BOOL fClearDirty);
        STDMETHODIMP DLL_EXPORT GetSizeMax(__out ULARGE_INTEGER * pcbSize)
                         // Allow 24 bytes for version.
                         { pcbSize->QuadPart = 12*sizeof(WCHAR)+SizeMax(); return NOERROR; }

        // implementation

        DLL_EXPORT CPersistStream(IUnknown *punk, __inout HRESULT *phr);
        DLL_EXPORT ~CPersistStream();

        HRESULT DLL_EXPORT SetDirty(BOOL fDirty)
        ...
        STDMETHODIMP DLL_EXPORT GetClassID(__out CLSID *pClsid) PURE;

        // overrideable if you want
        // file version number.  Override it if you ever change format
        virtual DWORD DLL_EXPORT GetSoftwareVersion(void) { return 0; }
        ...
        virtual int DLL_EXPORT SizeMax() {return 0;}
        virtual HRESULT DLL_EXPORT WriteToStream(IStream *pStream);
        virtual HRESULT DLL_EXPORT ReadFromStream(IStream *pStream);
        //=========================================================================
...
STDAPI DLL_EXPORT WriteInt(IStream *pIStream, int n);

// inverse of WriteInt
STDAPI_(int) DLL_EXPORT ReadInt(IStream *pIStream, __out HRESULT &hr);
