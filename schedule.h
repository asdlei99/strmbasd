...
class DLL_EXPORT CAMSchedule : private CBaseObject
{
public:
    virtual DLL_EXPORT ~CAMSchedule();
    // ev is the event we should fire if the advise time needs re-evaluating
    DLL_EXPORT CAMSchedule( HANDLE ev );

    DWORD DLL_EXPORT GetAdviseCount();
    REFERENCE_TIME DLL_EXPORT GetNextAdviseTime();

    // We need a method for derived classes to add advise packets, we return the cookie
    DWORD_PTR DLL_EXPORT AddAdvisePacket( const REFERENCE_TIME & time1, const REFERENCE_TIME & time2, HANDLE h, BOOL periodic );
    // And a way to cancel
    HRESULT DLL_EXPORT Unadvise(DWORD_PTR dwAdviseCookie);
    ...
    REFERENCE_TIME DLL_EXPORT Advise( const REFERENCE_TIME & rtTime );

    // Get the event handle which will be set if advise time requires re-evaluation.
    HANDLE DLL_EXPORT GetEvent() const { return m_ev; }
...
private:
    ...
    DWORD_PTR DLL_EXPORT AddAdvisePacket( __inout CAdvisePacket * pPacket );
    ...
    void DLL_EXPORT Delete( __inout CAdvisePacket * pLink );// This "Delete" will cache the Link

// Attributes and methods for debugging
public:
#ifdef DEBUG
    void DLL_EXPORT DumpLinkedList();
#else
    void DLL_EXPORT DumpLinkedList() {}
#endif
