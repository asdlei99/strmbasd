...
class DLL_EXPORT CCache : CBaseObject {

    /* Make copy constructor and assignment operator inaccessible */

    DLL_EXPORT CCache(const CCache &refCache);
    DLL_EXPORT CCache &operator=(const CCache &refCache);
...
public:
    ...
    DLL_EXPORT CCache(__in_opt LPCTSTR pName,INT iItems);
    virtual DLL_EXPORT ~CCache();

    /* Add an item to the cache */
    CBaseObject* DLL_EXPORT AddToCache(__in CBaseObject *pObject);

    /* Remove an item from the cache */
    CBaseObject* DLL_EXPORT RemoveFromCache();

    /* Delete all the objects held in the cache */
    void DLL_EXPORT RemoveAll(void);

    /* Return the cache size which is set during construction */
    INT DLL_EXPORT GetCacheSize(void) const {return m_iCacheSize;};
};
