...
STDAPI_(DWORD) DLL_EXPORT GetBitmapSize(const BITMAPINFOHEADER *pHeader)
{
...
STDAPI_(const GUID) DLL_EXPORT GetTrueColorType(const BITMAPINFOHEADER *pbmiHeader)
{
...
STDAPI_(const GUID) DLL_EXPORT GetBitmapSubtype(const BITMAPINFOHEADER *pbmiHeader)
{
    ASSERT(pbmiHeader);
...
STDAPI_(WORD) DLL_EXPORT GetBitCount(const GUID *pSubtype)
{
    ASSERT(pSubtype);
...
int DLL_EXPORT LocateSubtype(const GUID *pSubtype)
{
    ASSERT(pSubtype);
    const GUID *pMediaSubtype;
    INT iPosition = 0;
